...
공통 원본은 `art-export/production/drawable-nodpi/`에 둔다. iOS SpriteKit과 Android Canvas가 같은 PNG를 읽으며, 런타임 필터링 없이 정수 배율로 표시한다.
...
SpriteKit과 Canvas 모두 보간 이동 대신 1–7 논리 픽셀 단위의 순간 이동과 50–180ms 유지 프레임을 사용한다. 따라서 애니메이션은 보이지만 픽셀 경계는 흐려지지 않는다.
