...
- SpriteKit: 저장 선택, 전투 세계, 튜토리얼, 장비 작업대, 과부하 입력, 파편·자석 회수 연출
...
전투에 들어가면 `자동 해체 관찰 → 과부하 사용 → 고철 10 획득 → 강화 절단날 구매`를 화면 안에서 안내한다. 장비 작업대의 절단날·리벳 코일·자석 바구니는 고철을 실제로 소비하고 피해량 또는 부품 보상을 바꾸며 즉시 저장된다. 탄환은 총구에서 현재 적까지 5–6개의 정수 위치를 지나간 뒤에만 피해 판정을 발생시킨다.
...
검증 항목은 PCG32 기준 출력, 공통 콘텐츠 디코딩, 로컬 저장/백업 복구, 저장 선택 설명, SKView 단일 루트, UIButton 부재, 360×800 논리 크기, SKShapeNode 부재, 튜토리얼·장비 작업대 노드다.
